...
class BasePlugin:
    ...
    def onMessage(self,Connection,Data):
        ...
        if (Connection == self.httpConnControlInfo):
            ...
            index = dataDecoded[position + 6: position + 7]

            position = dataDecoded.find("OnOff")
            power = dataDecoded[position + 8: position + 11]

            position = dataDecoded.find("Mode")
            mode = dataDecoded[position + 7: position + 12]

            position = dataDecoded.find("FanSpeed")
            fanspeed = dataDecoded[position + 11: position + 15]

            position = dataDecoded.find("Temperature")
            stemp = dataDecoded[position + 12: position + 14]
