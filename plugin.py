    ...
    def onCommand(self,Unit,Command,Level,Color):
        ...
        if Unit == 1:  # Aircon Mode
            if Command == 'Set Level':
                if Level == 0: # Aircon Mode is OFF
                    Domoticz.Log("Aircon is Off !")
                    urlConnect = 'http://climdom-d47b.local/api_chunghopserver?action=changeconfig&remote=1&onoff=OFF'
                    url.call(urlConnect)
                if Level == 10: # Aircon Mode is AUTO
                    Domoticz.Log("Aircon is in Auto Mode !")
                    self.call_url('http:////climdom-d47b.local/api_chunghopserver?action=changeconfig&remote=1&onoff=ON')
