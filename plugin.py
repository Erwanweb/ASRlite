...
<plugin key="ASRlite" name="AC Aircon Smart Remote" author="MrErwan" version="0.0.1" externallink="https://github.com/Erwanweb/ASRlite.git">
    ...
    <params>
        ...
        <param field="Mode2" label="ASR Index" width="200px" required="true" default=""/>
...
class BasePlugin:
    ...
    def onMessage(self,Connection,Data):
        ...
        if (Connection == self.httpConnControlInfo):
            ...
            if (Devices[1].nValue != self.powerOn or Devices[1].sValue != sValueNew):
                Devices[1].Update(nValue = self.powerOn,sValue = sValueNew)
            ...
            Devices[4].Update(nValue = 0,sValue = stemp)
    ...
    def buildCommandString(self):
        ...
        if (Devices[4].sValue < "16"):  # Set temp Lower than range
            Domoticz.Log("Set temp is lower than authorized range !")
            requestUrl = requestUrl + "16"
        elif (Devices[4].sValue > "30"):  # Set temp Lower than range
