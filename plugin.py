...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 6 not in Devices:
            Domoticz.Device(Name = "ASR Server ID",Unit=6,Type = 243,Subtype = 6,).Create()
            devicecreated.append(deviceparam(6,0,"1"))  # default is Index 1
    ...
    def onMessage(self,Connection,Data):
        ...
        if (Connection == self.httpConnControlInfo):
            ...
            # Server SR index
            Devices[6].Update(nValue = 0,sValue = id)
    ...
    def buildCommandString(self):
        # Minimal string: /api_chunghopserver?action=changeconfig&remote=1&onoff=ON

        # Select good Index of the ASR
        requestUrl = "/api_chunghopserver?action=changeconfig&remote="

        if (Devices[6].sValue == "1"):
           requestUrl = requestUrl + "1"
        elif (Devices[6].sValue == "2"):
