...
class BasePlugin:
    ...
    def onMessage(self,Connection,Data):
        ...
        if (Connection == self.httpConnControlInfo):
            ...
            mac = dataDecoded[position + 12: position + 29]

            position = dataDecoded.find("OnOff")
            power = dataDecoded[position + 8: position + 10]

            position = dataDecoded.find("Mode")
            mode = dataDecoded[position + 6: position + 10]

            position = dataDecoded.find("FanSpeed")
            fanspeed = dataDecoded[position + 7: position + 8]

            position = dataDecoded.find("Temperature")
            stemp = dataDecoded[position + 10: position + 13]

            Domoticz.Debug(
                "mac: " + mac + ";Power: " + power + "; Mode: " + mode + "; FanSpeed: " + fanspeed + "; AC Set temp: " + stemp)
